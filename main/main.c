...
static void show_merge_screen(mnemonic_t* new_m, mnemonic_type_t result_type) {
    ...
    ui_show_merge_process(
        mnemonic_words(current),  ca_hex,
        na_hex,
        ma_hex,   mnemonic_words(preview),
        on_merge_done);
...
static void on_we_cancel(void) {
    ...
    we_handle = NULL;
    ui_show_source(on_generate, on_enter_manual, on_scan_qr, on_show_state, on_finish, current != NULL);
}
...
static void on_scan_qr(void) {
    FATAL("QR scanning not implemented yet.");
}
...
static void on_we_complete(void) {
    ...
    if (!m) { ui_show_main(on_new_wallet, on_test_error); return; }
    if (current) {
...
static void on_12(void) { word_count = 12; ui_show_source(on_generate, on_enter_manual, on_scan_qr, on_show_state, on_finish, current != NULL); }
static void on_24(void) { word_count = 24; ui_show_source(on_generate, on_enter_manual, on_scan_qr, on_show_state, on_finish, current != NULL); }

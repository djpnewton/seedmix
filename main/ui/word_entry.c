...
word_entry_handle_t ui_word_entry_begin(unsigned total_words, ui_cb_t on_done, ui_cb_t on_cancel) {
    ...
    lv_obj_set_flex_align(c->match_container, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
...
static void delete_screen_async(void* ptr) {
    lv_obj_delete((lv_obj_t*)ptr);
}
